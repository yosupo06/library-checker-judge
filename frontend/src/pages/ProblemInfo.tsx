import { Button, Container, FormControl, MenuItem, Select, TextField, Typography } from '@material-ui/core';
import React, { useContext, useState } from 'react';
import { connect, PromiseState } from 'react-refetch';
import { RouteComponentProps, withRouter } from 'react-router-dom';
import library_checker_client, { authMetadata } from '../api/library_checker_client';
import { LangListRequest, LangListResponse, ProblemInfoRequest, ProblemInfoResponse, SubmitRequest } from "../api/library_checker_pb";
...
const ProblemInfo: React.FC<Props> = (props) => {
  ...
  const handleSubmit = (e: React.FormEvent) => {
    e.preventDefault();
    console.log(props)
    library_checker_client.submit(new SubmitRequest().setLang(lang).setProblem(props.match.params.problemId).setSource(source), authMetadata(auth?.state!))
      .then((resp) => {
  ...
  return (
    <Container>
      <Typography variant="h2" paragraph={true}><KatexRender text={problemInfoFetch.value.getTitle()} /></Typography>
...
export default connect<RouteComponentProps<{ problemId: string }>, Props>(props => ({
  problemInfoFetch: {
    comparison: null,
    value: library_checker_client.problemInfo(new ProblemInfoRequest().setName(props.match.params.problemId))
  },
  langListFetch: {
    comparison: null,
