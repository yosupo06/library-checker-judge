...
const Submissions: React.FC<Props> = props => {
  ...
  const [submissionOverviews, setSubmissionOverviews] = React.useState<
    SubmissionOverview[]
  >([]);
  ...
  const handleSubmit = (e: React.FormEvent) => {
    ...
    const request = new SubmissionListRequest()
      ...
      .setLang(langFilter)
      .setLimit(100);
    library_checker_client.submissionList(request).then(resp => {
      ...
      <form onSubmit={e => handleSubmit(e)}>
        <FormControl className={classes.formControl}>
          ...
          >
            ...
            {props.problemListFetch.fulfilled &&
              props.problemListFetch.value.getProblemsList().map(e => (
