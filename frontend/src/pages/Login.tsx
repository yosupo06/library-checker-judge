...
const Help: React.FC<Props> = (props) => {
  ...
  const [ loginStatus, setLoginStatus ] = React.useState<"none" | "wait" | "success" | "failed">("none")
  const handleSubmit = (e: React.FormEvent) => {
    e.preventDefault();
    setLoginStatus("wait")
    library_checker_client.login(new LoginRequest().setName(userName).setPassword(password))
      .then((resp) => {
        auth?.dispatch( { type: 'login', payload: { token: resp.getToken(), user: userName }} )
  ...
  return (
    <Container>
      <Typography variant="h2" paragraph={true}>Login</Typography>
      <form onSubmit={e => handleSubmit(e)}>
        <div>
          <TextField required label="User Name" value={userName} onChange={(e) => setUserName(e.target.value)} />
        </div>
        <div>
