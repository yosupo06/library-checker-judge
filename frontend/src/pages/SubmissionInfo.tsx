...
import { SubmissionInfoRequest, SubmissionInfoResponse } from "../api/library_checker_pb";
...
const SubmissionInfo: React.FC<Props> = (props) => {
  ...
  return (
    <Container>
      <Typography variant="h2" paragraph={true}>Submission Info #{overview?.getId()}</Typography>
...
export default connect<RouteComponentProps<{ submissionId: string }>, Props>(props => ({
  submissionInfoFetch: {
    comparison: null,
    value: library_checker_client.submissionInfo(new SubmissionInfoRequest().setId(parseInt(props.match.params.submissionId)))
  }
}))(SubmissionInfo);
