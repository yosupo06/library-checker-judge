...
for module in restapi database storage uploader judge executor integration langs migrator tools/rejudge cloudrun/monitoring-discord-webhook cloudrun/taskqueue-metrics; do
    if [[ -d "$module" ]]; then
        (cd "$module" && go vet ./...)
    fi
done
